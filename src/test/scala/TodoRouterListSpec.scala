...
class TodoRouterListSpec extends WordSpec with Matchers with ScalatestRouteTest with TodoMocks {
  ...
  private val doneTodo = Todo("1", "test", "test", true)
  private val pendingTodo = Todo("2", "pending_test", "pending_test", false)

  private val todos = Seq(doneTodo, pendingTodo)

  "TodoRouter" should {
    "return all the todos" in {
      val repository = new InMemoryTestRepository(todos)
